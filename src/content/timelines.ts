import batmanIcon from "../assets/images/batman.svg";

export const timelineQuarterLabels = ["", "Q1", "Q2", "Q3", "Q4", ""];

export const timelineYearsData = [
    {
      year: 2018,
      dataset: [60, 100, 600, 750, 950, 930],
      data: [
        {
          title: "Q1",
          description:
            "2018 Q1 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "2018 Q2 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
  
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "2018 Q3 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "2018 Q4 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
      ],
    },
    {
      year: 2019,
      dataset: [60, 200, 750, 950, 950, 930],
      data: [
        {
          title: "Q1",
          description:
            "2019 Q1 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "2019 Q2 is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
      ],
    },
    {
      year: 2020,
      dataset: [60, 100, 750, 750, 950, 930],
      data: [
        {
          title: "Q1",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
      ],
    },
    {
      year: 2021,
      dataset: [60, 300, 930, 750, 600, 100],
      data: [
        {
          title: "Q1",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
      ],
    },
    {
      year: 2022,
      dataset: [60, 150, 550, 650, 950, 930],
      data: [
        {
          title: "Q1",
          description:
            "Ant-Man and the Wasp, comic strip superheroes created for Marvel Comics by Stan Lee and Jack Kirby. Ant-Man debuted in Tales to Astonish no. 27 (January 1962), and the Wasp first appeared in Tales to Astonish no. 44 (June 1963).",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "Batman, American comic strip superhero created for DC Comics by writer Bill Finger and artist Bob Kane. Batman debuted in May 1939 in Detective Comics no. 27 and has since appeared in numerous comic books, comic strips, and graphic novels; ",
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "Doctor Strange, American comic-book superhero created for Marvel Comics by writer Stan Lee and artist Steve Ditko.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "Spider-Man, comic-book character who was the original everyman superhero. In Spider-Man’s first story, in Marvel Comics’ Amazing Fantasy, no. 15 (1962), American teenager Peter Parker, a poor sickly orphan, is bitten by a radioactive spider.",
          img: batmanIcon,
        },
      ],
    },
    {
      year: 2023,
      dataset: [60, 200, 600, 750, 200, 100],
      data: [
        {
          title: "Q1",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q2",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q3",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
        {
          title: "Q4",
          description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Ipsum is simplIpsum is simply dummy text of the printing and typesetting industry.",
          img: batmanIcon,
        },
      ],
    },
  ];